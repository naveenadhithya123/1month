 ...
 *
 * - Chat / Quiz / Summarize           -> Groq
 * - Vision / Embeddings / Speech      -> HF Router


 * - Image generation                  -> SVG poster generator or Pollinations fallback
 */
...
// Groq is the only chat provider used by this project.
...
async function chatCompletionWithGroq({
  ...
  try {
    ...
    if (/429|rate limit|quota|resource exhausted/i.test(message)) {
      throw new Error("Groq is busy or out of quota. Please wait a minute and try again.");
    }
...
export async function generateImageFromPrompt(prompt, options = {}) {
  ...
  // Pollinations is free and does not require an API key.
